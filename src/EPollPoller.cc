...
namespace myMuduo
{
    ...
    Timestamp EPollPoller::poll(int timeoutMs, ChannelList *activateChannels)
    {
        ...
        int saveError =
            errno; // errno是全局变量,也是每个线程独有的，可能被后续操作覆盖，因此需要保存到局部变量 saveError中。
        ...
        if (numEvents > 0)
        {
            ...
            if (numEvents == events_.size())
            {
                ...
                maxevents参数指定长度），用于存储本次轮询中触发的事件。 如果实际触发的事件数超过
                maxevents，超出的部分会被丢弃（不会被记录到数组中）。 epoll_wait并不会主动向
                events_中添加元素——它只是将事件写入 events_预先分配的内存中。 因此，events_的大小（容量）需要在调用
